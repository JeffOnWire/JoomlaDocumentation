...
### Screenshot recommendations — CM4.3 (images/articles/287/) — updated for the three-configuration redesign

1. The Astronomy Articles Blog menu item settings (Category Blog, Category: Astronomy, default options).
2. The frontend category blog page before any images are added, showing all three articles' short intros only.
3. Crab Nebula's Images and Links tab, with Intro Image set to the composite and Full Article Image left empty.
4. Jupiter's Images and Links tab, showing the same Intro Image / no Full Article Image pattern.
5. The category blog page after both intro images are added — Crab Nebula and Jupiter show thumbnails, Andromeda Galaxy doesn't yet.
6. Andromeda Galaxy's Images and Links tab, showing Full Article Image set with Image Class typed as <em>none</em>, and no Intro Image.
7. The Andromeda Galaxy article page — Full Article Image filling the width at the top, no floating.
8. The Crab Nebula article page — no image at the top, intro paragraph and Read More content unchanged from before.
9. The finished category blog page with all three articles in their final state — two with intro thumbnails, one without.
...
**CM4.2: real images and all six screenshots placed, no text changes needed.** Jeff tested this tutorial as-is ("no changes") and placed the three real prerequisite images plus all six planned screenshots.
- The real Napoleon III Telescope filename initially came through from Wikimedia/Joomla with a comma and an ampersand (`Napoleon_III_Telescope,_University_of_Notre_Dame_with_Warner_&_Swasey_Equatorial_Mount.jpg`) rather than the placeholder underscore-only name used in the draft — updated the Prerequisites `src`/`data-path` to match, HTML-escaping the ampersand as `&amp;`. Jeff then renamed the file himself to drop the comma and ampersand, back to the original underscore-only name — updated `src`/`data-path` again to match. Attribution's link is untouched either way, since it points at the real external Commons page (which does have the comma/ampersand) and isn't affected by local filename changes.
- All three telescope images are 1200×900, confirming they're still full-size/unresized as intended (the Resizing Images section is what shrinks one of them to 720px later in the tutorial) — Prerequisites thumbnail dimensions (240×180) were already correct for this aspect ratio, no change needed there.
- All six screenshots placed as `<figure>`/`<figcaption>` pairs at their corresponding steps, with real pixel dimensions: the Media component/Create New Folder button (on the "Click on images in the file tree" step), the Telescopes folder with its two sub-folders (on the sub-folder creation step), dragging an image into Optical Telescopes (on the "Upload more images by dragging and dropping" step), the item context menu (on the Context Menu section's own paragraph — this section has no numbered steps, so the figure sits directly after the paragraph rather than inside an `<li>`), the Resize screen with width set to 720px (on the resize step), and the uploaded/renamed PDF (on the rename step).

CM4.2 is now fully real end to end — no open items remain on this file.

**CM4.3: Menu Item Type value fixed for consistency.** "Create a Category Blog" listed the value as just <em>Category Blog</em>; Jeff pointed out the project convention (used everywhere else a Category Blog menu item is created, e.g. CM2.3, CM6.2, CM7.2) is the full path, <em>Articles &gt; Category Blog</em>. Fixed.

**CM4.3: real bug found — Crab Nebula's full 4.1 content (image, caption, list, table, bordered image) was rendering in its entirety on the category blog listing**, since it carries no Read More break, while Jupiter and Andromeda Galaxy (created fresh in this tutorial with one-sentence Article Text) looked fine. Jeff caught this from a real screenshot of the blog page and suggested the fix himself: add a short intro sentence to Crab Nebula, matching the style of the other two articles, and insert a Read More break right after it. Added a new step to "Create a Category Blog," between the menu item's Save & Close and viewing the site: open Crab Nebula, type an intro sentence above the existing embedded image (<em>The Crab Nebula is the expanding remnant of a supernova recorded by astronomers in 1054 AD.</em>), then use <strong>Read More</strong> from the <strong>CMS Content</strong> toolbar dropdown — same mechanism and wording as the dedicated 6.2 Read More tutorial, though 4.3 doesn't formally link forward to it as a prerequisite (6.2 comes later in the sequence), just uses it inline. Also added one Concepts sentence explaining the behaviour, with a "covered in more depth in 6.2" pointer.

**Fixed:** the Result text for that new step originally said "a short intro and a Read more link" — Jeff pointed out that's confusing since only Crab Nebula gets a Read More break, not all three articles. Simplified to "a short intro."

**CM4.3: "Set Different Images on Crab Nebula" — Media popup folder navigation.** Jeff found that clicking Select for Full Article Image opened the Media popup on <em>Files &gt; Telescopes</em> instead of <em>Images &gt; Astronomy</em> — the popup remembers whatever folder was last visited (in his case, left over from CM4.2 testing), so it doesn't reliably land on the right place. Added a Note to the Full Article Image Select step telling the reader to navigate to <strong>Images &gt; Astronomy</strong> in the file tree if needed, and a short pointer on the following Intro Image step that the popup should still be showing that folder from the previous step.

**CM4.3: alt text does not carry over or get prompted for on Intro/Full Article Image — corrected across the whole section.** Jeff tested both remaining assumptions in "Set Different Images on Crab Nebula" and found both wrong:
- Step 4 (selecting the already-uploaded Crab Nebula image as Full Article Image) claimed the alt text "carried over" — it doesn't. Fixed to a Note that it must be added manually, back in the editor, in the field beneath the image.
- Step 5 (uploading the Crab Nebula composite as Intro Image) implied alt text is entered during upload via an Additional Data form, the same as inserting an image into article text (4.1's pattern). Jeff found no such form appears here — added a Note explaining the difference, and reworded the sub-bullet to "back in the editor, add alternate text in the field beneath the image."
- Applied the same "back in the editor" phrasing to the equivalent Jupiter and Andromeda Galaxy steps for consistency, since they use the same Upload-then-alt-text mechanism (not separately tested by Jeff, but same UI pattern, so corrected proactively rather than leaving the same ambiguous wording in three more places).

**CM4.3: real, confirmed cause of the Full Article Image float/skew bug found and fixed.** Jeff tested and reported two real display problems from live screenshots: the composite Intro Image renders huge on the category blog listing (likely just how Joomla displays these fields — no width/height resize control exists for Intro/Full Article Image the way TinyMCE's Insert/Edit Image dialog has for inline images, so display size depends entirely on the source file and responsive CSS filling the container — not something to fix in the tutorial itself), and the Full Article Image was floating left on the article page, skewing the intro paragraph beside it. Jeff found the real cause: an <strong>Image Class</strong> field under Full Article Image (a free-text field, not a dropdown) that defaults to a site-wide global setting shown as placeholder text — in his case <em>Use Global (left)</em> — which floats the image unless overridden. Typing <em>none</em> into the field fixed it. Added a new step to "Set Different Images on Crab Nebula" right after the alt-text Note, instructing the reader to type <em>none</em> under Image Class.

**Open question for Jeff:** while testing, Jeff also removed the Crab Nebula image embedded in the article body (originally added in 4.1) and moved its caption text into the Full Article Image's own Caption field instead, saying it looked "better." This directly contradicts the tutorial's current step 7 result text ("...followed further down by the same close-up again, embedded in the body text back in 4.1") and raises a real design question: was this just live-site experimentation to see what looks best, or does he want 4.3's instructions themselves updated to remove/replace that body image (and if so, does the pedagogical point of "the same image can appear twice, independently, as body content and Full Article Image" still need making some other way)? **Jeff confirmed this is just experimentation for now** — "We will definitely have to make some changes, just not sure what changes to make yet." No file changes made; revisit once he's settled on a direction.

**CM4.3: real finding — a smaller (~360px) Intro Image looks much better than the 720px composite on the category blog listing.** Jeff re-uploaded a 360px version of the Crab Nebula composite specifically for the Intro Image and confirmed it displays much better than the original 720px version, consistent with the earlier hypothesis that Intro/Full Article Image fields have no resize control and just render the source file at native size. This surfaced a real design tension — Jupiter and Andromeda Galaxy were using the *same* uploaded image for both Intro Image and Full Article Image, but the ideal size differs by context — resolved below.

**CM4.3: full redesign — three articles, three single-field configurations, no floats.** Jeff settled on a direction resolving both open threads above (the body-image-removal question and the intro/full-article sizing tension) at once: rather than combining Intro Image and Full Article Image on the same article (which kept running into styling/floating quirks), each of the three articles now demonstrates exactly one field, deliberately avoiding any combination that caused trouble:
- **Crab Nebula:** Intro Image only (the composite, resized to 360px). No Full Article Image. The image embedded in the article's text from 4.1 stays exactly as it was, appearing after the Read More break.
- **Jupiter:** Intro Image only (360px). No Full Article Image, nothing embedded in the text — a fully text-only article beyond its intro thumbnail.
- **Andromeda Galaxy:** Full Article Image only, used at its original width (960px, no resizing needed) so it fills the article's full width on its own. No Intro Image, nothing embedded in the text. Still needs <strong>Image Class: none</strong> (the float-default fix found earlier) since this is the one article actually using Full Article Image.

Restructured the file accordingly:
- Rewrote the overview paragraph — no longer "two the same, one different"; now "each article uses a single, different configuration, to avoid floating issues."
- Rewrote Prerequisites to describe per-image target sizes (360px for Jupiter and the Crab Nebula composite, 960px original width for Andromeda Galaxy) instead of one blanket 720px note.
- Replaced "Set Different Images on Crab Nebula" and "Set Matching Images on Jupiter and Andromeda Galaxy" with two new sections grouped by mechanic instead of by article: <strong>"Add an Intro Image to Crab Nebula and Jupiter"</strong> (same mechanic, two articles) and <strong>"Add a Full Article Image to Andromeda Galaxy"</strong> (different mechanic, one article, includes the Image Class fix).
- **Removed the "Float the Intro Image" section entirely.** It was never tested/confirmed all session (a long-standing open item), and Jeff's new design explicitly avoids floating altogether — so rather than leave an unconfirmed, likely-fragile section in, it's dropped, mirroring how CM4.1 dropped its own float section after real testing revealed problems. This also closes out CM4.3's own "Float option unconfirmed" open item — not by confirming it, but by removing the need to.
- Rewrote the first Concepts paragraph to reflect "one field per article, deliberately" rather than "commonly the same image for both." Added a new closing Concepts paragraph per Jeff's request: both fields render at whatever size is uploaded, with no built-in resize/crop control, so thoughtful, consistent sizing per use case (thumbnail vs. hero image) is what achieves a tidy result.
- Attribution unchanged — same three source images, just processed to different final sizes.

This closes out both previously-open questions (body-image removal, and the intro/full-article sizing tension) and the long-standing Float-option open item. Real per-size prerequisite images (Jupiter 360px, Crab Nebula composite 360px, Andromeda Galaxy 960px) and all screenshots for this redesigned version are still pending from Jeff.

**CM4.3: first round of Jeff's own revision pass — Setup section, simplified Select steps, trimmed results.** Jeff placed the three real, correctly-sized prerequisite images at `images/articles/287/` (Jupiter.jpg 360×360, Crab_Nebula_Composite.jpg 360×343, Andromeda_Galaxy.jpg 960×635 — matching his planned sizes exactly) and sent a first batch of structural/wording edits, explicitly expecting more rounds as he iterates:
- **Added a new "Setup" h2 section** between Prerequisites and Steps, moving the image-download bullet out of Prerequisites entirely. This mirrors an existing pattern already used in this project (CM6.2 Read More has its own "Setup" h2 before "Steps"). Setup now: a one-line intro ("We will be using the following images in the tutorial"), the three image previews (no per-image sizing explanation — Jeff asked to drop that detail), then three steps: download the images, open Content &gt; Media and navigate to Images &gt; Astronomy, upload the three images there. This means all three images are pre-uploaded before any article editing begins, extending the "upload once, select later" pattern already established in CM4.1.
- **"Create a Category Blog" step 4 Result** trimmed from "...but none has an image yet" to just "...with a short intro," since image assignment is now a separate later concern.
- **"Add an Intro Image to Crab Nebula and Jupiter" renamed to "Add Intro Images"**, with its explanatory intro paragraph replaced by one plain sentence defining what an intro image is.
- **Select steps simplified** throughout (Crab Nebula's and Jupiter's Intro Image, and Andromeda Galaxy's Full Article Image) from "click Select, then Upload the [image] you downloaded earlier" to "click Select, then click [filename.jpg] and Select" — since the images are already uploaded via Setup, no Upload action or popup-folder Note is needed anymore.
- **Removed "Leave Full Article Image empty"** (appeared twice, once per article) from "Add Intro Images" — no longer stated explicitly, just implied by omission.
- **Trimmed Result text**: the post-Select "View the site" step no longer says "Andromeda Galaxy doesn't have one yet"; the Crab Nebula click-through result now reads "The article appears as it did before. The intro image does not appear," with a Note that intro images only show in listings, not on the article's own page; the Jupiter click-through result now reads "No images appear — Jupiter is a text-only article."

**Not yet addressed, flagged for Jeff:** the parallel "Leave Intro Image empty" step in the Andromeda Galaxy section wasn't touched, since Jeff's instructions only named the two "Leave Full Article Image empty" steps in the Intro Images section — worth deciding if it should go too for consistency. Also, the Note warning that the Select popup might open to the wrong folder was removed along with the simplified wording; this is probably fine since Setup's Media Manager visit to Images &gt; Astronomy happens immediately beforehand in the same tutorial (unlike the CM4.2-vs-CM4.3 cross-tutorial case that caused the original bug), but worth watching for when Jeff tests this round.

**CM4.3: Jeff's second full self-rewrite — new Setup/article-creation structure, five real images, Attribution added.** Jeff restructured the file substantially beyond the previous round: Setup now downloads and uploads all five real images (Andromeda_Galaxy_Full.jpg 1280×720, Andromeda_Galaxy_Intro.jpg 480×270, Crab_Nebula_Intro.jpg 480×270, Jupiter_Full.jpg 1280×722, Jupiter_Intro.jpg 480×270) shown in a flex-row image grid, followed by a new "Create Additional Articles" section (creating the Jupiter and Andromeda Galaxy articles inline, in Setup, rather than assuming they exist), then Steps with "Create a Category Blog Menu," "Add Intro Images," "Add Full Article Images," and a new "Observe the Results" walkthrough section. Reviewed the whole file against the real files on disk and Jeff's request to "check for errors, style, and suggested changes":

- **Fixed directly (unambiguous):** three `data-path` attributes in the Setup image grid said `.png` for `Andromeda_Galaxy_Full`, `Andromeda_Galaxy_Intro`, and `Jupiter_Full` while `src` and the real files are `.jpg` — corrected to match. "Upload the three images" corrected to "Upload the images" (five now, not three). Grammar: "which appear at the top" → "which appears" (subject-verb agreement); Concepts' malformed "...screen readers, in which case you can check..." → "...screen readers, you can check..."; a sentence fragment in Observe the Results ("The same image in two files of different sizes.") completed. "Click on New" → "Click New." Capitalized "the Media Manager appears" after three "Result:" labels, and split each into two `<strong>` spans to match the pattern used elsewhere in the same file. Removed a stray empty `<li style="list-style-type: none;">` sitting alone between `<h2>Steps</h2>` and the first `<h3>`, apparently a leftover editing artifact. Also found and stripped 16 stray non-breaking-space characters scattered through the file (likely from copy/paste) that were invisible in rendering but occasionally broke exact-match tooling — replaced with regular spaces throughout.
- **Added the missing Attribution list** (the section had only its intro sentence, no citations) using Jeff's four supplied sources. Andromeda Galaxy's credit (Adam Evans, CC BY 2.0) is confirmed with confidence via search. The other three are best-available and **worth Jeff double-checking directly on the Commons pages** (our web-fetch tool still can't load commons.wikimedia.org pages this session, same limitation as CM4.1's Hubble image): Crab Nebula (noao-m1guvenen3) credited generically to NOIRLab/NOAO/AURA/NSF — search surfaced a possible individual photo processor name ("Guvenen") but not confirmed enough to put in print; Jupiter Storms Merging credited to NASA/JPL-Caltech/SwRI/MSSS with processing by Tanya Oleksuik, CC BY — this one has reasonably strong search corroboration (matches NASA's own JunoCam citizen-science credit line); Juice launch kit cover close-up credited generically to ESA — least-confirmed of the four, search couldn't reach the actual file page.
- **Flagged, not changed — two content questions for Jeff:**
  1. The Read More step for Crab Nebula (added earlier this session to fix the real "full article spills into category blog listing" bug) has been entirely removed from this rewrite, and Concepts now defers the whole topic to "a future tutorial." Crab Nebula's article body still has substantial embedded content (image, caption, list, table, bordered image) from 4.1, so without a Read More break it should reproduce the original bug on the category blog listing. Worth confirming this removal was intentional.
  2. Concepts still describes the Image Class: none fix for float/skew, but the current "Add Full Article Images" steps contain no such step for either Andromeda Galaxy or Jupiter. This may be fine now that both Full Article Images are large/full-width (Concepts says full-width images aren't a problem), but it's a real gap between what Concepts describes and what Steps does — worth confirming.
- **Style items surfaced, not changed (Jeff's call):** "Log in to the backend." sits outside any list, and doesn't match the rest of the project's "Log in to Joomla backend and navigate to the Home Dashboard." step-one wording. The whole file now consistently omits "Click" before "Save & Close"/"Save & New" (e.g. just "**Save & Close**") — a real departure from the "Click **Save & Close**" phrasing used everywhere else in the project; flagging as a global consistency question rather than changing unilaterally. "Add Full Article Images" steps are terser than the parallel "Add Intro Images" steps (no "Tick the checkbox...Result:" pattern) — may be an intentional pacing choice. The `<div style="display: flex;">` layout for the Setup image grid is a one-off styling technique not used elsewhere in the project. Figcaptions in the Setup grid ("Andromeda Full," "Jupiter Intro") are terser than this project's usual descriptive-sentence figcaptions — reads fine for a "here are the files to grab" grid, just noting the tone shift.

**CM4.3: Jeff's decisions on the two flagged content questions, plus three new style rules.**

- **Read More removal confirmed intentional.** Jeff reviewed the live category blog listing and found it's not as problematic as first thought — the standardized 16:9 intro images stand out clearly above the article detail, and even where article content starts appearing further down, the eye follows the repeating image+detail pattern rather than getting lost. He was also uncomfortable introducing Read More before it's formally covered in 6.2. No change needed — the removal stays as Jeff left it.
- **Image Class: none instruction removed from Concepts** (the "Typing *none* in the image's style setting will prevent this" sentence) — Steps never uses it in this version, so Concepts no longer references it either. Reworded the surrounding sentence to just note that full-width images, like the ones used here, aren't affected by the inline-floating behaviour, without describing the fix for a problem the tutorial no longer demonstrates.
- **"Log in" moved to a real first step.** The floating "Log in to the backend." paragraph (outside any list) was replaced with a proper first `<li>` in the Get the Images list: "Log in to Joomla backend and navigate to the **Home Dashboard**," matching the project's established phrasing (from CM4.2, etc.).
- **New style rule — log in once, as step 1.** Added to the Tutorial Style Guide: direct the user to log in only once per tutorial, as the very first step (in Setup if present, otherwise step 1 of Steps), using the standard "Log in to Joomla backend and navigate to the Home Dashboard" phrasing. Don't repeat the instruction elsewhere in the same tutorial.
- **New style rule — "Click X" first time, bare "X" after.** Jeff's decluttering idea: when the same action (like clicking Save & Close) is triggered more than once in a tutorial, write "Click" before it only the first time; every later occurrence of that same action drops "Click." Added to the Tutorial Style Guide with a Save & Close example. Applied within CM4.3 itself: the first-ever save action (Save & New, creating the Jupiter/Andromeda Galaxy articles) is now "Click **Save & New**," and the first Save & Close (closing out article creation) is now "Click **Save & Close**." — every later Save & Close in the file was already bare and needed no change.
- **Attribution credits corrected with confirmed licensing.** Jeff visited the three uncertain Commons pages directly and supplied their actual license text, replacing the earlier best-guess/search-sourced credits:
  - Crab Nebula (noao-m1guvenen3): NOIRLab, **CC BY 4.0** (NOIRLab's site-wide license for its public images).
  - Jupiter Storms Merging: **public domain** — NASA-created content isn't copyrighted unless explicitly noted; credited to NASA/JPL-Caltech/SwRI/MSSS (dropped the earlier "Tanya Oleksuik, CC BY" guess, which conflated a different citizen-scientist-processed version with this file's actual page).
  - Juice Launch Kit Cover (close-up): ESA, **CC BY-SA 3.0 IGO**.

**Backlog — not acting on now, revisit later (per Jeff, to avoid getting distracted from CM4.3):**
- **Visual restructuring of steps.** Jeff finds the current single-line step format (action, then bolded Result:, then sometimes a bolded Note:) hard to scan back into after looking away to perform the action — he has to re-scan the whole list to find his place, then hunt within the line for the Result/Note parts. He'd like to explore a more visually distinct treatment for Result/Note — a sub-bullet, blockquote, alert-style box, or similar — that makes it easier to resume a list after looking away. If a good pattern is found, this could become a batch-overhaul pass across all tutorials rather than a one-off change. No design decided yet.
- **Standardize the flex-div image-grid layout.** Jeff likes how `<div style="display: flex;">` spaces out and reflows the Setup download-image grid (used for the first time in this CM4.3 rewrite) and wants to standardize it as the project's pattern for "here are the files to download" galleries — but not implement it retroactively right now.
- **PDF thumbnail for CM4.2's downloadable file.** CM4.2 has an images-and-PDF Prerequisites section; Jeff wants to eventually generate a thumbnail image for the PDF so it can sit in the same kind of image grid as the other downloadable files, rather than being a plain link. Not started.
- Jeff may track these as GitHub issues going forward instead of (or alongside) this notes file.
